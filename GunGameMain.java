...
public class GunGameMain extends JavaPlugin {
	...
	public void onEnable() {
		...
		this.saveConfig();
		getLogger().info("[GunGame] Was successfully loaded!");
			...
			if (sender instanceof Player) {
				...
				if (!p.hasPermission("gungame.admin")) {
					...
					if (args.length == 0) {
						...
						p.sendMessage("§6/gga setlobby <ID>§8to set the arenalobby");
						p.sendMessage("§6/gga addspawn <ID> §8to add an arenaspawn");
						p.sendMessage("§6/gga resetspawns <ID> §8to reset the arenaspawns");
						p.sendMessage("§6/gga addarena <ID> §8to create an arena!");
						return true;
					...
					if (args[0].equalsIgnoreCase("setmainspawn")) {
						...
						p.sendMessage(prefix + "§aThe main spawn for GunGame was successfully set!");
					...
					if (args[0].equalsIgnoreCase("setlobby")) {
						...
						if (args.length == 1) {
							...
							p.sendMessage(prefix + "§aSuccessfully updated the LobbyLocation of §6" + arenaID + "!");
						}
					...
					if (args[0].equalsIgnoreCase("addspawn")) {
						...
						if (args.length == 1) {
							...
							p.sendMessage(prefix + "§aSuccessfully added a SpawnLocation of §6" + arenaID + "!");
						}
					...
					if (args[0].equalsIgnoreCase("resetspawns")) {
						...
						if (args.length == 1) {
							...
							Arena a = this.manager.getArena(arenaID);
							a.resetSpawns();
							p.sendMessage(prefix + "§aSuccessfully reset SpawnLocations of §6" + arenaID + "!");
						}
					}
				}
